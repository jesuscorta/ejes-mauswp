...
const initHeaderSearch = () => {
  ...
  const renderResultItem = (item) => {
    const imageHtml = item.image_url
      ? `<img src="${escapeHtml(item.image_url)}" alt="" class="header-search__result-thumb" loading="lazy">`
    ...
    const categoryHtml = item.category
      ? `<span class="header-search__result-category">${escapeHtml(item.category)}</span>`
      : '';

    return `
      <a href="${escapeHtml(item.url)}" class="header-search__result-item" role="option" data-header-search-item>
        ${imageHtml}
        <div class="header-search__result-body">
          <h3 class="header-search__result-title">${escapeHtml(item.title)}</h3>
