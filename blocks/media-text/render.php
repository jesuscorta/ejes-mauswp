...
$block_classes = [ 'media-text-block', $align_class ];
...
if ( '' === $eyebrow ) {
	$eyebrow = __( 'Ejes para remolques', 'mauswp' );
}
...
if ( '' === $content ) {
	$content = '<p>' . esc_html__( 'Bloque editorial preparado para presentar producto, capacidad técnica y argumentos de venta con una imagen de apoyo y un contenido claro.', 'mauswp' ) . '</p>';
}
...
>
	<div class="media-text-block__inner">
		<div class="media-text-block__media">
			<?php if ( $image_id > 0 ) : ?>
				<?php echo wp_get_attachment_image( $image_id, 'large', false, [ 'class' => 'media-text-block__image', 'alt' => $image_alt, 'loading' => 'lazy', 'decoding' => 'async' ] ); ?>
			<?php elseif ( '' !== $image_url ) : ?>
				<img class="media-text-block__image" src="<?php echo esc_url( $image_url ); ?>" alt="<?php echo esc_attr( $image_alt ); ?>" loading="lazy" decoding="async">
			<?php elseif ( ! empty( $is_preview ) ) : ?>
				<div class="media-text-block__placeholder" aria-hidden="true"></div>
			<?php endif; ?>
		</div>

		<div class="media-text-block__content">
			<p class="media-text-block__eyebrow"><?php echo esc_html( $eyebrow ); ?></p>
			<h2 class="media-text-block__title"><?php echo esc_html( $title ); ?></h2>
			<div class="media-text-block__copy">
				<?php echo wp_kses_post( $content ); ?>
			</div>

			<?php if ( '' !== $cta_url && '' !== $cta_label ) : ?>
				<a
					class="media-text-block__cta"
